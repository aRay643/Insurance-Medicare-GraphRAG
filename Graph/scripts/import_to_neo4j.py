"""
JSON 三元组数据导入 Neo4j 数据库脚本 (V2)
========================================
将 Medicine_v2、Insurance_v2、NursingHome_v2 三个目录下的 JSON 三元组数据
导入到 Neo4j 图数据库中，实现跨域自然关联和数据去重。

V2 改进：
- 指向 V2 版本数据目录（经过 V6 Prompt 重新提取）
- 新增 ProductCategory / Service 节点索引
- 跨域桥接关系（养老服务 → 保险类别）
- 保留完整去重、UNWIND 批量导入、验证报告等功能
"""
...
try:
    ...
    HAS_TQDM = False
    print("⚠ 未安装 tqdm，将使用简单进度显示。可运行 pip install tqdm 安装。")
...
DATA_SOURCES = {
    "medicine": os.path.join(BASE_DIR, "db_data", "Medicine_v2"),
    "insurance": os.path.join(BASE_DIR, "db_data", "Insurance_v2"),
    "nursing_home": os.path.join(BASE_DIR, "db_data", "NursingHome_v2"),
}
...
CROSS_DOMAIN_BRIDGES = [
    ...
    (
        "专业护理 → 护理保险",
        """
        MATCH (s:Service {name: "专业护理"})
        MATCH (pc:ProductCategory {name: "护理保险"})
        MERGE (s)-[:SUITABLE_FOR]->(pc)
        """
    ),
    (
        "机构养老 → 养老年金保险",
        """
        MATCH (s:Service {name: "机构养老"})
        MATCH (pc:ProductCategory {name: "养老年金保险"})
        MERGE (s)-[:SUITABLE_FOR]->(pc)
        """
    ),
    (
    ...
    (
        "含医疗设施机构 → 补充医疗保险",
        """
        MATCH (o:Org) WHERE o.has_medical_facility = true
        MATCH (pc:ProductCategory {name: "补充医疗保险"})
        MERGE (o)-[:SUITABLE_FOR]->(pc)
        """
    ),
]
...
def build_cross_domain_bridges(session):
    """
    建立跨域桥接关系（养老服务 → 保险类别）。
    """
    print("\n🔗 建立跨域桥接关系...")
    total_created = 0
    for desc, cypher in CROSS_DOMAIN_BRIDGES:
        try:
            ...
            if created > 0:
                print(f"  ✓ {desc}: 创建 {created} 条关系")
            else:
                print(f"  ○ {desc}: 无新关系（节点可能不存在）")
        except Exception as e:
            print(f"  ⚠ {desc}: {e}")
    print(f"  共创建 {total_created} 条跨域桥接关系")
...
def verify_import(session):
    ...
    # 跨域路径：养老机构 → 服务 → 保险类别 → 保险产品
    result = session.run(
        "MATCH path = (o:Org)-[:PROVIDES_SERVICE]->(s:Service)"
    ...
    if record:
        print(f"  养老机构→服务→保险类别→保险产品 路径数: {record['cnt']:,}")
    ...
    result = session.run(
        ...
        "RETURN labels(n)[0] AS label, n.name AS name LIMIT 5"
    )
    records_list = list(result)
    if records_list:
        print("  种子数据:")
...
def main():
    ...
    print("=" * 60)
    print("🚀 JSON 三元组 → Neo4j 导入工具 (V2)")
    print("=" * 60)
    print(f"  Neo4j URI:  {NEO4J_URI}")
    print(f"  数据库:     {NEO4J_DATABASE}")
    print(f"  数据目录:   {BASE_DIR}")
    print(f"  清空数据库: {'是' if clear_db else '否'}")
    print()
    ...
    for domain, directory in DATA_SOURCES.items():
        print(f"📂 加载 {domain} 数据 ({directory})...")
                ...
                if HAS_TQDM:
                    ...
                    for i in range(0, len(triplets), BATCH_SIZE):
                        ...
                        if batch_num % 10 == 0 or batch_num == total_batches:
                            ...
                            print(f"    进度: {batch_num}/{total_batches} 批次 | {elapsed:.1f}s")

            # 6. 跨域桥接
            build_cross_domain_bridges(session)
